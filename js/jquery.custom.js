(function($){
    ...
    $(document).ready(function(){
		...
		var ut_word_rotator = function() {
			...
			var ut_rotator_words = $('.hero-title-text').map(function(){ return $(this).html(); }) ,
			counter = 0;
		...
		if( !device.tablet() && !device.mobile() ) {
			...
			$('#program').addClass('parallax-banner-3');
		...
		$('#to-o-projektu').click( function(event) {
		...
		var streamWidth = Math.min($(window).width()-12, 900);
		var streamHeight = Math.floor(streamWidth*0.5625)-4;

		$('.livestream').magnificPopup({
			type: 'iframe',
			...
			iframe: {
				patterns: {
					livestream: {
						index: 'live.php',
						id: function(url) {
							console.log($('.mfp-content').width());
							return 'konsek';
						},
						src: 'http://cdn.maksnet.tv/embed/livetv01/?streamname=%id%&width='+streamWidth+'&height='+streamHeight
					}
				}
			}
		...
		function animate_sections() {
			...
			$('.wrap').waypoint(function(direction) {
				...
				if( direction === 'down' && !$(this).hasClass('animated') ) {
					...
					setTimeout ( function () {
							$('#o-projektu header').find('.section-title').animate({ opacity: 1 });
					}, 200 );

					setTimeout ( function () {
							$('#o-projektu header').find('.section-slogan').animate({ opacity: 1 });
					}, 600 );
			...
			/* team section */
			$('#predavaci').waypoint( function( direction ) {

				if( direction === 'down' && !$(this).hasClass('animated') ) {

					setTimeout ( function () {
							$('#predavaci header').find('.section-title').animate({ opacity: 1 });
					}, 200 );

					setTimeout ( function () {
							$('#predavaci header').find('.section-slogan').animate({ opacity: 1 });
					}, 600 );
			...
			$('#program').waypoint( function( direction ) {

				if( direction === 'down' && !$(this).hasClass('animated') ) {

					setTimeout ( function () {
							$('#program header').find('.parallax-title').animate({ opacity: 1 });
					}, 200 );

					setTimeout ( function () {
							$('#program header').find('.parallax-slogan').animate({ opacity: 1 });
					}, 600 );
			...
			/* sponzori*/
			$('#sponzori').waypoint( function( direction ) {

				if( direction === 'down' && !$(this).hasClass('animated') ) {

					setTimeout ( function () {
							$('#sponzori header').find('.section-title').animate({ opacity: 1 });
					}, 200 );

					setTimeout ( function () {
							$('#sponzori header').find('.section-slogan').animate({ opacity: 1 });
					}, 600 );
